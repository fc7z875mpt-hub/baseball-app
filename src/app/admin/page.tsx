...
export default function AdminPage() {
  ...
  async function onLogoFile(file: File | null, teamId?: string) {
    if (!file) return;
    try {
  ...
  function clubsOf(user: UserRow): string {
    ...
    for (const p of user.players) for (const t of p.teams || []) names.add(t.team.shortName || t.team.name);
  ...
  return (
    <main className="min-h-screen bg-[#0a1628] px-4 py-8 text-white">
      <div className="mx-auto max-w-3xl">
                ...
                <div key={team.id} className="flex items-center justify-between gap-3 rounded-2xl border border-white/10 bg-white/5 p-4">
                  <div className="flex items-center gap-3">
                    {team.logoUrl ? (
                      ...
                      <div className="flex h-10 w-10 items-center justify-center rounded-full text-sm font-bold text-white" style={{ backgroundColor: team.primaryColor }}>{(team.shortName || team.name).slice(0, 2).toUpperCase()}</div>
                    )}
                    <div>
                      <p className="font-semibold">{team.name}</p>
                      <p className="text-xs text-white/50">{team.shortName || "—"} · {team._count?.players ?? 0} hráčů{!team.isActive && " · neaktivní"}</p>
                    </div>
                  </div>
                  <div className="flex flex-col items-end gap-2 sm:flex-row sm:items-center">
                    <label className="cursor-pointer rounded-lg border border-white/15 px-3 py-1.5 text-xs text-white/70 hover:bg-white/5">Logo<input type="file" accept="image/png,image/jpeg,image/webp" className="hidden" onChange={(e) => onLogoFile(e.target.files?.[0] || null, team.id)} /></label>
                    <button onClick={() => toggleTeam(team.id, team.isActive)} className="rounded-lg border border-white/15 px-3 py-1.5 text-xs text-white/70">{team.isActive ? "Deaktivovat" : "Aktivovat"}</button>
                  </div>
                </div>
              ))}
            </div>
        ...
        {tab === "promote" && (
          <>
            <div className="mb-4 rounded-2xl border border-amber-500/20 bg-amber-500/5 p-4 text-sm text-amber-100/80">Systém navrhne postup (U8→U9…). Odškrtni hráče, kteří zůstávají. Filtry jen omezují zobrazení.</div>
            ...
            <div className="mb-4 space-y-2">
              {filteredPromote.length === 0 && <p className="py-8 text-center text-white/40">Žádní hráči v tomto filtru</p>}
              {filteredPromote.map((p) => {
                const willPromote = !skipIds.has(p.id) && !!p.suggestedCategory && p.suggestedCategory !== p.category;
                const teamLabel = (p.teams || []).map((t) => t.shortName || t.name).join(", ") || "—";
                return (
